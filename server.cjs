...
app.post('/api/admin-request-reset', async (req, res) => {
  ...
  const resetLink = `${process.env.DEV_SITE_ORIGIN || 'https://www.franceparts.be/'}/admin/reset?token=${encodeURIComponent(token)}&email=${encodeURIComponent(admin.email)}`;
...
app.get('/api/_preview-reset-email', (req, res) => {
  ...
  const resetLink = `${process.env.DEV_SITE_ORIGIN || 'https://www.franceparts.be/'}/admin/reset?token=${encodeURIComponent(token)}&email=${encodeURIComponent(email)}`;
